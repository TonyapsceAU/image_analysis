...
def find_speech_bubbles(image_path):
  ...
  gray = cv2.cvtColor(image,
                      cv2.COLOR_BGR2GRAY)  # Convert the image to grayscale
  _, binary = cv2.threshold(
    gray, 0, 255, cv2.THRESH_BINARY_INV +
    cv2.THRESH_OTSU)  # Apply adaptive thresholding to obtain a binary image
...
def check_overlap(rect1, rect2):
  ...
  if (x1 < x2 + w2 * 1, 25 and x1 + w1 * 1, 25 > x2 and y1 < y2 + h2 * 1, 25
      and y1 + h1 * 1, 25 > y2):
    return True
...
def draw_distribution_curve(rects):
  sorted_rects = sorted(rects, key=lambda rect: rect[2] * rect[3]
                        )  # Sort rectangles based on area (width * height)
...
def output_images_from_rectangles(image_path, rectangles, output_prefix):
  ...
  cnt = len(os.listdir(
    output_dir))  # Get the number of existing images in the output directory
...
for filename in os.listdir(input_folder_path):
  if filename.endswith('.jpg') or filename.endswith('.png'):
    ...
    if len(rectangles):
      ...
      output_images_from_rectangles(image_path, rectangles, output_folder_path)
