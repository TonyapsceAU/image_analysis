...
# upload_path = "upload"
temp_path = "websitev4/temporary"
app.secret_key = "s1e2a3n4"
app.config['UPLOAD_FOLDER'] = 'websitev4/static/upload'  # Folder to store uploaded images
...
def processed_images():
    ...
    for image_name in origenal_images_path:
        ...
        image_quadtree.check_img("websitev4/temporary/"+image_name.split('.')[0]+"_0",image)
        words_images_path = image_quadtree.get_images()
        tes(combine(words_images_path))
        for words_image_path in words_images_path:
            word_image = Image.open("websitev4/temporary/"+words_image_path)
